        ...
        <div class="bg-white p-8 rounded-lg shadow-lg dark:border-gray-700 dark:bg-gray-900">
            ...
            <form action="{{ route('booking.accept', $booking->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="check_in_date" class="block text-lg font-medium text-gray-700 dark:text-gray-300">Check
                            in
                            date</label>
                        <input type="text" value="{{ \Carbon\Carbon::parse($booking->check_in_date)->format('M-d-Y') }}"
                            name="check_in_date" id="check_in_date"
                            class="mt-2 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-md p-3 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-600"
                            readonly>
                    </div>
                    <div>
                        <label for="check_out_date" class="block text-lg font-medium text-gray-700 dark:text-gray-300">Check
                            out
                            date</label>
                        <input type="text" value="{{ \Carbon\Carbon::parse($booking->check_out_date)->format('M-d-Y') }}"
                ...
                <div data-group-id="3">
                    <div>
                        <label for="payment-3" class="block text-lg font-medium text-gray-700 dark:text-gray-300">Payment
                            Amount</label>
